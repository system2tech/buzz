...
// the backfill query. `reconcileInboundPersonaEvent` is not safely idempotent —
// re-applying the same 30177 head re-triggers a runtime deployment (see
// `coalesceManagedAgentBackfill`) — so the drain drops any buffered event whose
// id the backfill already reconciled. Steady-state events post-drain carry ids
// the backfill never saw, so they are unaffected.
//
...
export function startPersonaSync(
  ...
  // is delivered live (buffered) AND returned by the backfill. `reconcile` is
  // not idempotent, so the drain skips any buffered event already reconciled by
  // the backfill. Populated during backfill, consulted once at drain.
