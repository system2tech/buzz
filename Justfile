...
test-unit:
    ...
    if command -v cargo-nextest &>/dev/null; then
        ...
        # buzz-agent's model-capability corpus, OAuth auth coordinator matrix,
        # and Databricks integration tests are infra-free. Run all targets so
        # the library drift guard and lock/cooldown/crash-recovery coverage all
        # execute remotely; nothing in CI otherwise tests the workspace crate.
