...
/// Result-level read authorization for events whose content or envelope is
/// private to a single viewer. Currently gates `KIND_DM_VISIBILITY`,
/// `KIND_AGENT_TURN_METRIC`, and `KIND_WORKFLOW_MENTION_WAKE`: the reader MUST
/// equal the event's `#p` tag (owner/recipient). Returns `true` for every other kind.
///
...
pub fn reader_authorized_for_event(event: &nostr::Event, reader_pubkey_hex: &str) -> bool {
    ...
    if kind != crate::kind::KIND_DM_VISIBILITY
        ...
        && kind != crate::kind::KIND_WORKFLOW_MENTION_WAKE
    {
        return true;
    }
