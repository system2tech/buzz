...
/// on the same branch (S2).
///
/// Named rather than version-gated, deliberately. The gate below implements an
/// early revision of ACP RFD #1237, which proposed the protocol version as the
/// capability signal; the RFD has since moved to a capability advertised at
/// `initialize`, and nothing here follows it yet. Meanwhile `s2harness` answers
/// `protocolVersion: 1` because v1 is what it implements -- and that honest answer
/// used to cost it its instructions entirely: this gate suppressed the
/// user-message fallback believing the field had been delivered, while the harness
/// could not read the field it was sent. Measured 2026-09-01 -- the agent ran on a
/// stock prompt and could not name itself.
///
/// Replace both names with the RFD's capability handshake once it lands.
...
fn session_new_system_prompt<'a>(
    ...
    // parsing and the agent never sees it (measured 2026-09-01, a request carrying
    // both arrived with only the `_meta` half). (S2)
...
mod tests {
    ...
    /// the version gate read that as "cannot take a system prompt" while ALSO
    /// suppressing the user-message fallback — so the channel's agent instructions
    /// reached the model through neither path.
    #[test]
    fn s2harness_gets_the_system_prompt_via_meta_on_protocol_v1() {
        ...
        // An unknown harness on v1 is unchanged: no field, so `format_prompt`
