...
- **Answering someone IS needing their attention.** When your message replies to what a person asked you, address them: put their `@Name` in the content and pass `--mention <their hex pubkey>` in the same command. The `<context>` block gives you both for the triggering event's author. A thread reply without this notifies nobody — no Activity entry, no unread on mobile, no push — because all three key off the mention, not off the thread you replied in. Replying in their thread is not enough.
