...
- **Top-level channel messages notify members automatically** — you do not need to `@mention` someone just because you are replying to them. A top-level post in a channel they are in is enough. Only `@mention` in a reply when you need to cut through noise (e.g., the channel has many members and your message is specifically for one person) or when posting inside a thread (thread replies do not notify unless you mention).
