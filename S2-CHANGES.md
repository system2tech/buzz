...
thread gate, wakes on every thread reply in every visible channel under `joined` as much
as `room`; `persistent: true` on a Claude Code Monitor not surviving a session
