...
- **IPs, instance sizes, location, OS/data volumes, SSH keys:** the team's Verda project and each instance's attached volumes. Record server roles in Verda's names/descriptions so detached volumes remain identifiable. Server addresses are deliberately not recorded in this repository; obtain them from Verda or from a team member with console access.
