...
### Accept the bypass-permissions disclaimer — human, once, interactively

Signing in is not sufficient. The manager runs `claude --bg --permission-mode bypassPermissions`, which refuses until that disclaimer has been accepted once in your own account. So run it yourself, on the agent server, as your own account, and accept:

```bash
"$HOME/mrfix/bin/claude" --dangerously-skip-permissions
```
...
manager launch failed; will retry the same identity
```

The supervisor is behaving correctly there — it retries the same identity rather than inventing a new one — so the loop is harmless and will resolve the moment the disclaimer is accepted.

> Found on 2026-09-09 by Harri's agent onboarding his manager, and missing from this section until then for an instructive reason: **the accounts that built this tooling had already accepted the disclaimer by hand long before**, so the check silently passed for everyone who wrote the instructions. A prerequisite your own environment satisfies invisibly is one you will not think to document.
